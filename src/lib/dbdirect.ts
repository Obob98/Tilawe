...
export async function fetchClients(): Promise<Client[]> {
    ...
    try {
        ...
        return data.map(({ firstname, lastname, contact, address, email }) => ({ firstname, lastname, contact, address, email }))
    } catch (error) {
        console.error('Database Error:', error);
        throw new Error('Failed to fetch revenue data.');
    }
...
export async function fetchSuppliers(): Promise<Supplier[]> {
    ...
    try {
        ...
        return data.map(({ name, contact, address }) => ({ name, contact, address }))
    } catch (error) {
        console.error('Database Error:', error);
        throw new Error('Failed to fetch revenue data.');
    }
...
export async function fetchIPurchasedtems(): Promise<FetchPurchasedItemsReturnType[]> {
    ...
    try {
        ...
        return data.map(({ item_id, purchase_transaction_id, quantity, unit_price }) => ({ item_id, purchase_transaction_id, quantity, unit_price: formatCurrency(Number(unit_price)) }))
    } catch (error) {
        console.error('Database Error:', error);
        throw new Error('Failed to fetch revenue data.');
    }
